...
module.exports.email = async (value, checkDisposable) => {
    ...
    if (value != "") {
        if (!regex.test(value)) {
            logger.error('[Regex > Email]: Incorrect Email Value. Not match with regex');
            return false;
        }
        if (checkDisposable) {
            ...
            if (body.disposable) {
                logger.error('[Regex > Email]: Disposable Email found');
                return false;
            }
        ...
        logger.error('[Regex > Email]: Incorrect Email Value. Value not Found');
        return false;
    }
}
